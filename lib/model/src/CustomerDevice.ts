...
function sanitize(customerDevice: Model, hasId: boolean): Model {
  ...
  if (
    !isString(customerDevice.customer_id) ||
    customerDevice.customer_id.trim() === ""
  ) {
    const error: HttpError.Model = {
      status: 400,
  ...
  if (
    !isString(customerDevice.device_id) ||
    customerDevice.device_id.trim() === ""
  ) {
    const error: HttpError.Model = {
      status: 400,
...
const sanitizeExistingCustomerDevice = (
  customerDeviceExis: Model,
...
export {
  Model,
  sanitize,
  sanitizeExistingCustomerDevice,
  sanitizeIdExisting,
