...
function sanitize(branch: Model, hasId: boolean): Model {
  ...
  if (
    !isString(branch.phone_number) ||
    !isValidPhoneNumber(branch.phone_number)
  ) {
    const error: HttpError.Model = {
      status: 400,
      message:
        'Invalid or missing "phone_number". It must be a number between 9 and 15 digits.',
    };
    throw error;
  }
  if (
    branch.additional_phone &&
    (!isString(branch.additional_phone) ||
      !isValidPhoneNumber(branch.additional_phone))
  ) {
    const error: HttpError.Model = {
      status: 400,
      message:
