...
// GET(customer_id)
export const getMonthlyPaymentByCustomerId = async (customer_id: string,page =1): Promise<PaginatedMonthlyPayments> => {
    try {
        const newToken = await handleTokenRefresh();
        if (!newToken) {
            return { data: [], page, totalPages: 0, total: 0 };
        ...
        const response: AxiosResponse<PaginatedMonthlyPayments> = await axios.get(`${baseUrl}/customer/${customer_id}?page=${page}`, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            },
...
// GET(status)
export const getMonthlyPaymentByStatus = async (status: 'active' | 'inactive',page=1): Promise<PaginatedMonthlyPayments> => {
    try {
        const newToken = await handleTokenRefresh();
        if (!newToken) {
            return { data: [], page, totalPages: 0, total: 0 };
        ...
        const response: AxiosResponse<PaginatedMonthlyPayments> = await axios.get(`${baseUrl}/status/${status}?page=${page}`, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            },
...
// GET(organization)
export const getMonthlyPaymentByOrganization = async (organization: string,page=1): Promise<PaginatedMonthlyPayments> => {
    try {
        const newToken = await handleTokenRefresh();
        if (!newToken) {
            return { data: [], page, totalPages: 0, total: 0 };
        ...
        const response: AxiosResponse<PaginatedMonthlyPayments> = await axios.get(`${baseUrl}/organization/${organization}?page=${page}`, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            },
