...
const generateAccessTokenTranzila = (
  appPublicKey: string,
  appPrivateKey: string,
): { headers: { [key: string]: string }; time: number; nonce: string } => {
  const time = Math.round(new Date().getTime() / 1000)
  const nonce = makeid(80)
  const hash = CryptoJS.HmacSHA256(appPublicKey, appPrivateKey + time + nonce).toString(
    CryptoJS.enc.Hex,
  )
  const headers = {
