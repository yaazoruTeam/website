...
const createItem = async (item: ItemForMonthlyPayment.Model, trx?: DatabaseTransaction) => {
...
const updateItem = async (item_id: string, item: ItemForMonthlyPayment.Model, trx?: DatabaseTransaction) => {
  const knex = getDbConnection()
  try {
    const query = trx ? trx('yaazoru.item') : knex('yaazoru.item')
...
const deleteItem = async (item_id: string, trx?: DatabaseTransaction) => {
  const knex = getDbConnection()
  try {
    const query = trx ? trx('yaazoru.item') : knex('yaazoru.item')
