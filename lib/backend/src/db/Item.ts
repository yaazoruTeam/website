...
const createItem = async (item: ItemForMonthlyPayment.Model, trx?: KnexTransaction) => {
...
const updateItem = async (item_id: string, item: ItemForMonthlyPayment.Model, trx?: KnexTransaction) => {
  const knex = getDbConnection()
  try {
    const query = trx ? trx('yaazoru.item') : knex('yaazoru.item')
...
const deleteItem = async (item_id: string, trx?: KnexTransaction) => {
  const knex = getDbConnection()
  try {
    const query = trx ? trx('yaazoru.item') : knex('yaazoru.item')
