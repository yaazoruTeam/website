import { Device, HttpError, DatabaseTypes } from '@model'
import getDbConnection from '@db/connection'
import config from '@config/index'
const limit = config.database.limit

const createDevice = async (device: Device.Model, trx?: DatabaseTypes.DatabaseTransaction) => {
...
const findDevice = async (criteria: {
  device_id?: string
  SIM_number?: string
  IMEI_1?: string
...
const doesDeviceExist = async (device_id: string): Promise<boolean> => {
  const knex = getDbConnection()
  try {
    const result = await knex('yaazoru.devices').select('device_id').where({ device_id }).first()
    return !!result
  } catch (err) {
    throw err
  }
