...
const createBranchCustomerSchema = async () => {
  logger.debug('create branchCustomer schema')

  const knex = getDbConnection()
  try {
    const tableExists = await knex.schema.withSchema('yaazoru').hasTable('branchCustomer')
    if (!tableExists) {
      logger.debug('Creating branchCustomer table...')
      await knex.schema
        .withSchema('yaazoru')
      ...
      logger.debug('branchCustomer table created successfully.')
    } else {
      logger.debug('branchCustomer table already exists. Skipping creation.')
    }
  } catch (err) {
