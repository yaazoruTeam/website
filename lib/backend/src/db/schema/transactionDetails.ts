...
const createTransactionDetailsSchema = async () => {
  ...
  try {
    const tableExists = await knex.schema.withSchema('yaazoru').hasTable('transactionDetails')
    if (!tableExists) {
      ...
      console.log('transactionDetails table already exists. Skipping creation.')
    }
  } catch (err) {
