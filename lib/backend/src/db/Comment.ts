...
const getComments = async (offset: number): Promise<{ comments: Comment.Model[], total: number }> => {
    const knex = getDbConnection();
    try {
...
const getCommentsByEntity = async (entity_id: string, entity_type: string, offset: number): Promise<{ comments: Comment.Model[], total: number }> => {
    const knex = getDbConnection();
    try {
        const comments = await knex('yaazoru.comments')
        ...
        const [{ count }] = await knex('yaazoru.comments')
            .where({ entity_id, entity_type })
            .count('*');
...
const updateComment = async (comment_id: string, comment: Partial<Comment.Model>): Promise<Comment.Model> => {
    const knex = getDbConnection();
    try {
