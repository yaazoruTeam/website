...
import { CustomerDevice, HttpError } from '@/model/src'
...
const getAllDevicesByCustomerId = async (
  ...
  try {
    ...
    const allCustomerDevice: CustomerDevice.Model[] =
      await db.CustomerDevice.getCustomerDeviceByCustomerId(req.params.id)
