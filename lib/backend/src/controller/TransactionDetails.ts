...
const createTransactionDetails = async (
  ...
  try {
    ...
    const sanitized: TransactionDetails.Model = TransactionDetails.sanitize(
      transactionDetailsrData,
      false,
    )
    //
    // chargeTokenTranzila();
...
const getTransactionDetailsById = async (
  ...
  try {
    ...
    const transactionDetails = await db.TransactionDetails.getTransactionDetailsById(req.params.id)
    res.status(200).json(transactionDetails)
  } catch (error: any) {
    next(error)
  }
...
const deleteTransactionDetails = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const deleteTransactionDetails = await db.TransactionDetails.deleteTransactionDetails(
      req.params.id,
    )
