...
const getCommentsByEntity = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const { comments, total } = await db.Comment.getCommentsByEntity(entity_id as string, entity_type as string, offset);
