...
const terminateMobile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result: Widely.Model = await callingWidely('prov_terminate_mobile', {
...
const provResetVmPincode = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const result: Widely.Model = await sendMobileAction(endpoint_id, 'prov_reset_vm_pincode')
