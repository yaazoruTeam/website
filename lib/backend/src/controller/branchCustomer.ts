...
const createBranchCustomer = async (
  ...
  try {
    ...
    const existCustomer = await db.Customer.doesCustomerExist(
    ...
    const existCombination =
      await db.BranchCustomer.doesBranchCustomerCombinationExist(
        sanitized.branch_id,
        sanitized.customer_id
      );
    if (existCombination) {
    ...
    const branchCustomer = await db.BranchCustomer.createBranchCustomer(
      sanitized
    );
...
const getBranchCustomerById = async (
  ...
  try {
    ...
    const existBranchCustomer = await db.BranchCustomer.doesBranchCustomerExist(
      req.params.id
    );
    ...
    const branchCustomer = await db.BranchCustomer.getBranchCustomerById(
      req.params.id
    );
...
const getBranchCustomerByBranch_id = async (
  ...
  try {
    ...
    const branchCustomer = await db.BranchCustomer.getBranchCustomerByBranch_id(
      req.params.id
    );

    if (!branchCustomer || branchCustomer.length === 0) {
      const error: HttpError.Model = {
        status: 404,
        message: "no branch customers found for this branch.",
      };
      throw error;
    }
...
const getBranchCustomerByCustomer_id = async (
  ...
  try {
    BranchCustomer.sanitizeIdExisting(req);
    const existCustomer = await db.BranchCustomer.doesCustomerExist(
      req.params.id
    );
    if (!existCustomer) {
      const error: HttpError.Model = {
    ...
    const branchCustomer =
      await db.BranchCustomer.getBranchCustomerByCustomer_id(req.params.id);

    if (!branchCustomer || branchCustomer.length === 0) {
      const error: HttpError.Model = {
        status: 404,
        message: "No branchCustomers found for this customer.",
      };
      throw error;
    }
...
const deleteBranchCustomer = async (
  ...
  try {
    ...
    const deleteBranchCustomer = await db.BranchCustomer.deleteBranchCustomer(
      req.params.id
    );
