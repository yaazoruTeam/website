...
describe('Comments Controller', () => {
  ...
  it('should fail to create comment with missing fields', async () => {
    ;(db.Comment.createComment as jest.Mock).mockImplementation(() => {
  ...
  it('should update comment', async () => {
    ;(db.Comment.updateComment as jest.Mock).mockResolvedValue({
      ...mockComment,
      content: 'Updated',
    })
    const res = await request(app)
      .put('/comments/1')
      .send({ ...mockComment, content: 'Updated' })
    expect(res.status).toBe(200)
  ...
  it('should return 404 when deleting non-existing comment', async () => {
    ;(db.Comment.deleteComment as jest.Mock).mockImplementation(() => {
